...
class JWTBearer(HTTPBearer):
    ...
    async def verify_jwt(self, token: str) -> tuple[bool, AccessTokenPayload | None]:
        try:
            payload: AccessTokenPayload = AccessTokenPayload(**JWTHandler.decode(token=token))

            access_token_key = CacheTag.USER_ACCESS_TOKEN.value.format(user_id=payload.user_id)
            user_data_key = CacheTag.USER_DATA.value.format(user_id=payload.user_id)
            if (
                token != await ASYNC_CACHE_MANAGER.get(key=access_token_key)
                or payload.sub != "access"
                or not await ASYNC_CACHE_MANAGER.get(key=user_data_key)
            ):
                return False, None
            return True, payload

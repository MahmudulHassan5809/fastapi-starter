        ...
        try:
            ...
            if data["success"]:
                data["data"] = (
                    ...
                    and response_json.get("data") is not None
                    else response_json
                )

                if request.url.path == "/api/v2/settings/public/app-settings/":
                    data["data"]["is_system_in_maintenance"] = getattr(
                        request.state,
                        "is_bypass",
                        data["data"]["is_system_in_maintenance"],
                    )
            ...
            data["meta_info"] = (
                response_json.get("meta", {}) if isinstance(response_json, dict) else {}
            )

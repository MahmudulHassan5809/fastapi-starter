    ...
    def __call__(self, config: HttpRequestConfig) -> dict[str, Any]:
        try:
            request_params = {
                ...
                "timeout": 5,
                config.payload_key_type: config.payload,
            }
            ...
            logger.info("send request response: %s", {response.content})
            json_response = cast(dict[str, Any], response.json())

            if response.status_code != 200:
                raise RequestError(message=f"HTTP error {response.status_code}: {json_response}")
